...
- **Testing**: Use pytest with pytest-django for all tests. Place tests in `tests/` directory.
...
- **Virtual Models**: All virtual models must inherit from `VirtualModel` and set `managed = False`
- **Managers**: Use `VirtualManager` for QuerySet-like operations
- **QuerySet Compatibility**: Ensure virtual QuerySets behave like Django QuerySets where possible
- **Admin Integration**: Virtual models should work with Django admin when appropriate
- **Migration Path**: Provide clear migration path for existing tools
